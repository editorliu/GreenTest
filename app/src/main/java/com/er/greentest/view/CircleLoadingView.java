...
public class CircleLoadingView extends View {
    ...
    public CircleLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        baseColor = tya.getColor(R.styleable.CircleLoading_baseColor, Color.BLACK);
    ...
    private void drawArcScale(Canvas canvas) {
        ...
        int rx = mWidth / 2;
        int ry = mHeight / 2;
        int radius = Math.min(rx, ry);
        Log.w("KGH", "rx:" + rx + "  ry:" + ry+" radius:"+radius);

        canvas.drawCircle(rx,ry,radius,mScalePaint);
...
//            canvas.drawLine(mWidth / 2, 0, mWidth / 2, DensityUtil.dip2px(mContext, 10), mScalePaint);
//            canvas.drawLine(mWidth / 2, 0, mHeight / 2, DensityUtil.dip2px(mContext, 10), mScalePaint);
            if (i == 1) {
                Log.w("KGH", "rx:" + rx + "  ry:" + ry+" ry-radius+DensityUtil.dip2px(mContext, 10):"+(ry-radius+DensityUtil.dip2px(mContext, 10)));
            }
            canvas.drawLine(rx, ry-radius, rx, ry-radius+DensityUtil.dip2px(mContext, 10), mScalePaint);
            // 旋转的度数 = 100 / 360
//            canvas.rotate(3.6f, mWidth / 2, mHeight / 2);
            canvas.rotate(3.6f, rx, ry);
        }
     ...
     * 设置进度伴随动画
     */
    public void setProgressWithAnimator(int progress) {
        isDot = false;
        if (progress <= 0) {
            return;
        }
